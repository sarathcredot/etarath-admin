...
const EditProduct = ({ isOpen, toggle, productId }: Props) => {
  // STATES
  ...
  const handleEditProduct = async (values: any) => {
    try {
      ...
      let uploadedIndex = 0;
      values.imageUrl = values.imageUrl.map((img: any) => {
        if (typeof img === "string") {
          return img; // keep existing URL
        } else {
          return uploadedUrls[uploadedIndex++]; // replace file with uploaded URL
        }
      });

      console.log(values, "VALUES = = = ");
      const res = await updateProduct({ id: productId, data: values });
