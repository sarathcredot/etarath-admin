...
const AddAuctionPlan = ({ isOpen, toggle }: Props) => {
  //USEFORMIK
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      ...
      handleAddAuctionPlan(values);
    },
  });

  //HANDLERS
  //MUTATION
    // const { mutateAsync: addAuctionPlan } = useCreateAuctionPlan();
  
    //HANDLERS
    const handleAddAuctionPlan = async (values: any) => {
      try {
        // const res = await addAuctionPlan(values);
    ...
    <>
      <Modal show={isOpen} onHide={toggle} centered={true}>
        <Modal.Header>
          ...
          <h3 className="my-2">Add Auction Plan</h3>
        </Modal.Header>
