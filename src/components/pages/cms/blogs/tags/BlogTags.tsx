...
import {
  useDeleteBlogTag,
  useGetAllBlogTags,
} from "src/services/blog.tag.service";
import AddBlogTag from "./popups/AddBlogTag";
...
const BlogTags = () => {
  ...
  // MUTATIONS
  const { mutateAsync: deleteBlogTag } = useDeleteBlogTag();
  ...
  const handleDeleteBlogTag = async () => {
    ...
    try {
      const res = await deleteBlogTag({ tag: selectedTag });
      if (res) {
  ...
  return (
    <>
      ...
      <AddBlogTag isOpen={isOpen} toggle={() => setIsOpen(!isOpen)} />
      <ConfirmationPopup
        submit={() => handleDeleteBlogTag()}
        isOpen={isDeleteOpen}
