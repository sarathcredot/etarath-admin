...
import AddOffer from "./popups/AddOffer";
import {
  useDeleteOffer,
  useGetOffers,
  useUpdateOfferStatus,
} from "src/services/offer.service";
import { useGetAdds, usestatusUpdateAdds, usestatusDeleteAdds } from "src/services/adds.service"
import { generateFilePath } from "src/services/url.service";
import EditOffer from "./popups/EditOffer";
...
const OffersPage = () => {
  ...
  // QUERIES
  const { data: offers, isLoading } = useGetAdds();

  // MUTATIONS
  const { mutateAsync: updateAddsStatus } = usestatusUpdateAdds();
  const { mutateAsync: deleteAdd } = usestatusDeleteAdds();
  ...
  const limit = 10;
  const totalRecords = offers?.length || 0;
  ...
  const handleStatusChange = async (id: string, status: boolean) => {
    try {
      if (id) {
        ...
        const res = await updateAddsStatus({ id, status });
        if (res) {
  ...
  return (
    <>
      ...
      <AddOffer isOpen={isOpen} toggle={() => setIsOpen(!isOpen)} />
      <EditOffer
      ...
      <ConfirmationPopup
        submit={() => handleDeleteOffer(selectedOffer?._id)}
        isOpen={isDeleteOpen}
        toggle={() => setIsDeleteOpen(!isDeleteOpen)}
        text={"Are you sure that you want to delete this advertisement ?"}
      />
    </>
  );
};
